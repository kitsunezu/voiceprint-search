...
``AudioPreprocessor.process()`` which enforces a deterministic sequence:
    ...
    1. Vocal / music separation  (Demucs / MDX) on the original input
    2. FFmpeg normalise          (mono 16 kHz s16 WAV)
...
class PreprocessResult:
    ...
    playback_source_path: str           # higher-quality source kept for playback/archive output
...
class AudioPreprocessor:
    ...
    def process(
        ...
        *,
        ...
        # 1. Vocal separation (strip background music) on the source audio.
        if use_separation:
            step_started = time.perf_counter()
            source_path, sep_dir = self.separator.separate(raw_wav_path)
            cleanup_dirs.append(sep_dir)
            separation_applied = source_path != raw_wav_path
            if collect_timings:
                timings["separate"] = round(time.perf_counter() - step_started, 4)

        # 2. Normalise the post-separation source for VAD / embeddings.
        step_started = time.perf_counter()
        ...
        speech = self.vad.extract_speech(
            ...
            fallback_to_raw=not separation_applied,
        )
        if collect_timings:
        ...
        return PreprocessResult(
            ...
            playback_source_path=source_path,
            total_speech_seconds=total_speech_seconds,
            timings=timings,
            options={
                "separate_vocals": separation_applied,
                "denoise": use_denoise,
            },
        ), cleanup_dirs
